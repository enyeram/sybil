...
groups_df = pd.read_csv(r"D:\code\moonbeam_large_connected_groups.csv")
...
for index, group in enumerate(groups, start=1):
    ...
    for address in addresses:
        transactions = get_transactions_moonbeam(address, API_KEYS_MOONBEAM[0])
        for tx in transactions:
            ...
            if tx["from"] in addresses and tx["to"] in addresses and tx["from"] != tx["to"]:
                transfer_str = (f'hash: "{tx["hash"]}" from: "{tx["from"]}" to: "{tx["to"]}" '
                                f'tokenSymbol: "{tx["tokenSymbol"]}" value: "{int(tx["value"]) / (10 ** 18)}"')
...
# save
output_file_path = r"D:\code\moonbeam_token_transfers_per_group.csv"
