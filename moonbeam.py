...
def get_transactions_moonbeam(address, api_key):
    ...
    if current_time - last_request_times[api_key] < 1:  # Less than a second since last request
        ...
        if request_counters[api_key] >= 3:  # Adjust this based on Moonscan's rate limit
            time.sleep(0.2)
...
df_related_addresses.to_csv(r"D:\code\moonbeam_related_address_pairs.csv", index=False)
